...
#' 
mi_fam_d_hel <- dist.ldc(mc_chrys, "hellinger", silent = T)
...
#' 
mi_fam_jac <- vegdist(mc_chrys, method = 'jac', binary = T)
...
#' 
betadiver(mc_chrys, method = 'j') %>% tidy
#' 
...
#' 
mi_fam_sor <- vegdist(mc_chrys, method = 'bray', binary = T)
mi_fam_sor %>% tidy
