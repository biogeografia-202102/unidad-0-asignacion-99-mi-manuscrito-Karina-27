...
nrow(mc_chrys) #En la matriz de comunidad
...
specnumber(mc_chrys)
sort(specnumber(mc_chrys)) # Ordenados ascendentemente
...
summary(colSums(mc_chrys)) # Resumen estadístico

#' ### Riqueza numérica de toda la "comunidad"
specnumber(colSums(mc_chrys))
...
abun_sp_q <- crear_grafico_mosaico_de_mc(mc_chrys, tam_rotulo = 6)
abun_sp_q
