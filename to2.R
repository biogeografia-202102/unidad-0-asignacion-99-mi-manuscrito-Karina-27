...
#' 
env_ambiental <- bci_env_grid %>%
  st_drop_geometry %>%
  dplyr::select_if(is.numeric) %>%
  dplyr::select(-id, -matches('^[A-T,Z]|pH', ignore.case = F))
...
env_selec2 <- bci_env_grid %>%
  select(
    ...
    geomorf_hombrera_pct,
    heterogeneidad_ambiental,
    Al, B, Ca, Cu, Fe, K, Mg, Mn, Zn, N, N.min., pH) %>% 
  st_drop_geometry
...
env_selec3 <- bci_env_grid %>%
  select(
    elevacion_media,
    geomorf_hombrera_pct,
    Al, B, Ca, Cu, Fe, K, Mn, Zn, N, N.min., pH) %>% 
  st_drop_geometry
